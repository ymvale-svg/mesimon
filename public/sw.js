...
self.addEventListener('fetch', (event) => {
  ...
  /**
   * קוד: קודם רשת, ורק אם אין רשת — מהמטמון.
   *
   * כאן היה באג שהעלים כל תיקון. קודם הוגש קודם המטמון, והרשת רק עדכנה
   * אותו לפעם הבאה — בהנחה ש"קובץ חדש מגיע בטעינה הבאה, וזה מקובל לקבצי
   * מערכת". ההנחה שגויה, כי ה-HTML כן נטען מהרשת: אחרי העלאת גרסה התקבל
   * HTML חדש עם JavaScript ישן, וכל תיקון בשרת שדרש גם שינוי בלקוח נראה
   * כתקלה חדשה. כך "נשבר" החיפוש — לקוח ישן קרא ‎data.projects‎ מתשובה
   * שהשרת החדש מחזיר כ-‎data.results‎, וההצגה נבלעה בשקט.
   *
   * העלות היא המתנה לרשת בטעינת קבצי הקוד. זה המחיר הנכון: אפליקציה
   * שנפתחת ברבע שנייה ומריצה קוד שאינו תואם לשרת אינה נפתחת מהר, היא
   * מוצגת שבורה. ללא רשת המטמון עדיין מגיש הכול, וזו כל תכליתו.
   */
  ...
  /**
   * שאר הנכסים — סמלים, תמונות, גופנים: קודם מטמון, והרשת מעדכנת לפעם
   * הבאה. הם אינם משתנים יחד עם הלוגיקה, ולכן גרסה מאחור בהם אינה שוברת דבר.
   */
  event.respondWith((async () => {
