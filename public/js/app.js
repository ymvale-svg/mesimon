...
const App = (() => {
  ...
  async function showDesktopNotification(n) {
    ...
    const options = {
      ...
      tag: `mesimon-${n.id}`,
      /*
       * ההתראה נעלמת מעצמה, ואינה נשארת על המסך עד שנוגעים בה.
       *
       * ‎requireInteraction: true‎ היה כאן בכוונה — התראה שמגיעה כשאדם בפגישה
       * ונעלמת לא הודיעה לו דבר. בפועל התוצאה הייתה ערימת התראות שמכסה את
       * הפינה ודורשת סגירה ידנית אחת-אחת, וזה גרוע יותר: מי שנתקל בערימה
       * מנקה אותה בלי לקרוא.
       *
       * אין דרך לקבוע משך מדויק מהדפדפן — הוא נקבע בהגדרת Windows
       * ("ביטול התראות לאחר", ברירת מחדל חמש שניות). ‎false‎ מוסר את
       * ההצמדה ומחזיר את השליטה למשתמש ולמערכת ההפעלה שלו, וההתראה ממילא
       * נשמרת במרכז ההתראות של Windows ואינה אובדת.
       */
  ...
  async function sendTestNotification() {
    ...
    const options = {
      ...
      // כמו התראה אמיתית — בדיקה שמתנהגת אחרת אינה בדיקה
      requireInteraction: false, data: { taskId: null }
    };
    try {
      const reg = await navigator.serviceWorker?.ready;
      if (reg) {
